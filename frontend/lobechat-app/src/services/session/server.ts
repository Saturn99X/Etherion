...
import { getThreads } from '@/etherion/bridge/session';
import { LobeSessionType, type ChatSessionList, type LobeSessions } from '@/types/session';
...
export class ServerService implements ISessionService {
  ...
  getGroupedSessions: ISessionService['getGroupedSessions'] = () => {
    return (async (): Promise<ChatSessionList> => {
      const threads = await getThreads();

      const sessions: LobeSessions = threads.map((thread) => {
        const base = DEFAULT_AGENT_LOBE_SESSION;
        const now = new Date();
