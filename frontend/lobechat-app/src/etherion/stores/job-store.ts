...
interface Job {
  ...
  threadId?: string;
  pendingApprovalPayload?: BlueprintApprovalPayload;
...
interface JobStore {
  ...
  setJobField: (jobId: string, field: keyof Job, value: any) => void;
  addExecutionStep: (jobId: string, step: ExecutionStep) => void;
...
export const useJobStore = create<JobStore>((set, get) => ({
  ...
  updateJob: (jobId: string, update: Partial<JobStatusUpdate>) => {
    set((state) => {
      ...
      const updatedJob: Job = {
        ...
        isPendingApproval: nextStatus === 'PENDING_APPROVAL',
  ...
  setJobField: (jobId: string, field: keyof Job, value: any) => {
    set((state) => {
      const existingJob = state.jobs[jobId];
      if (!existingJob) return state;
      return {
        jobs: {
          ...state.jobs,
          [jobId]: { ...existingJob, [field]: value },
        },
      };
    });
  },

  addExecutionStep: (jobId: string, step: ExecutionStep) => {
    set((state) => {
      const existingJob = state.jobs[jobId];
      if (!existingJob) return state;

      // Avoid duplicates based on timestamp and title
      const isDuplicate = existingJob.executionTrace.some(
        (s) => s.timestamp === step.timestamp && s.title === step.title
      );
      ...
      return {
        jobs: {
          ...
          [jobId]: {
            ...existingJob,
            executionTrace: [...existingJob.executionTrace, step],
          },
        },
      };
    });
  ...
  subscribeToJob: (jobId: string) => {
    ...
    try {
      ...
      const { apolloClient } = require('@etherion/ui/layout/apollo-provider');
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const { SUBSCRIBE_TO_JOB_STATUS, SUBSCRIBE_TO_EXECUTION_TRACE } = require('@etherion/lib/graphql-operations');
      ...
      const sub2 = apolloClient.subscribe({
        ...
        next: ({ data }: any) => {
          if (data?.subscribeToExecutionTrace) {
            ...
            const evtType = (evt.status || add.type || '').toString().toUpperCase();
            ...
            if (evt.current_step_description) {
              const toolDetails = add.tool_call || add.toolCall;
              get().addExecutionStep(jobId, {
                title: evt.status || 'Step',
                summary: evt.current_step_description,
                status: (evt.status?.toLowerCase() === 'completed' ? 'completed' :
                  evt.status?.toLowerCase() === 'running' ? 'running' : 'pending') as any,
                timestamp: evt.timestamp || new Date().toISOString(),
                toolCall: toolDetails ? {
                  toolName: toolDetails.name || toolDetails.toolName,
                  arguments: typeof toolDetails.arguments === 'string' ? toolDetails.arguments : JSON.stringify(toolDetails.arguments),
                  output: toolDetails.output,
                } : undefined,
              });
            }

            // Detect BLUEPRINT_APPROVAL_REQUIRED event and store payload
            ...
            // Detect THREAD_CREATED or THREAD_SELECTED events to bind threadId
            if (evtType === 'THREAD_CREATED' || evtType === 'THREAD_SELECTED') {
              const newThreadId = add.thread_id || add.threadId;
              if (newThreadId) {
                set((state) => {
                  const existingJob = state.jobs[jobId];
                  if (!existingJob) return state;
