...
import { sendGoalAndStream } from '@/etherion/bridge/chat';
import { setThreadModel } from '@/etherion/bridge/models';
...
> = (set, get) => ({
  sendMessageInServer: async ({ message, files, onlyAddUserMessage, isWelcomeQuestion }) => {
    ...
    get().internal_toggleSendMessageOperation(operationKey, true);
    ...
    if (threadId && provider && model) {
      setThreadModel(threadId, undefined, provider, model);
    }

    // Helper: clear all loading indicators after stream finishes or errors.
    const clearLoading = () => {
    ...
    try {
      const result = await sendGoalAndStream({
        threadId,
        text: message,
        provider,
        model,
        planMode: chatConfig.planMode,
        // Stream completion: clear loading and update final message content
        onFinish: async () => {
          clearLoading();
        },
        // Stream error: surface error, clear loading
        onError: (error) => {
          const errorMessage =
            typeof (error as any)?.message === 'string'
              ? (error as any).message
              : 'Stream error';
          get().internal_updateSendMessageOperation(operationKey, { inputSendErrorMsg: errorMessage });
          clearLoading();
        },
      });

      // Replace the operation's AbortController with the SSE stream controller so
      // cancelSendMessageInServer() aborts the live stream, not just the submission.
      get().internal_updateSendMessageOperation(
        operationKey,
        { abortController: result.streamController },
        'creatingMessage/stream-started',
      );

      // Loading is NOT cleared here — we wait for onFinish / onError.
      return;
    } catch (e: any) {
      // sendGoalAndStream threw synchronously (auth failure, network error, etc.)
      console.error('Etherion sendGoalAndStream error:', e);
      const errorMessage = typeof e?.message === 'string' ? e.message : 'Failed to send message';
      get().internal_updateSendMessageOperation(operationKey, { inputSendErrorMsg: errorMessage });
