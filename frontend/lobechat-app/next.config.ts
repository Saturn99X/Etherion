...
const analyzer = process.env.ANALYZE === 'true' ? require('@next/bundle-analyzer') : null;
...
const nextConfig: NextConfig = {
  ...
  webpack(config) {
    ...
    // Add lobechat-app's node_modules as an absolute fallback for ALL module lookups.
    // Required because vendor/ is a symlink to ../frontend/vendor which is outside this
    // project root; with symlinks=true (default), webpack follows the symlink and then
    // walks up from the real path, missing lobechat-app's node_modules. By adding the
    // absolute path here, vendor/ files can still find packages like zod, trpc, etc.
    config.resolve.modules = [
      ...(config.resolve.modules || ['node_modules']),
      path.resolve(__dirname, 'node_modules'),
    ];

    // Explicit aliases for project-root @/ paths.
    // Next.js's JsConfigPathsPlugin handles tsconfig paths, but when transpilePackages
    // compiles @lobechat/* workspace packages, those files also use @/ project aliases.
    // We add explicit aliases here to ensure reliable resolution:
    //   - Specific workspace package aliases (override the catch-all for those dirs)
    //   - A catch-all '@' alias for all other @/* imports (libs, app, components, etc.)
    config.resolve.alias = {
      ...config.resolve.alias,
      '@/const': path.resolve(__dirname, 'packages/const/src'),
...
const withBundleAnalyzer: (config: NextConfig) => NextConfig = analyzer ? analyzer() : noWrapper;
