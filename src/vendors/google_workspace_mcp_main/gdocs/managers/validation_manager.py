...
class ValidationManager:
    ...
    def validate_table_data(self, table_data: List[List[str]]) -> Tuple[bool, str]:
        ...
        if not table_data:
            return False, ("Table data cannot be empty. "
                           "Required format: [['col1', 'col2'], ['row1col1', 'row1col2']]")

        if not isinstance(table_data, list):
            return False, (f"Table data must be a list, got {type(table_data).__name__}. "
                           "Required format: [['col1', 'col2'], ['row1col1', 'row1col2']]")
        ...
        if not all(isinstance(row, list) for row in table_data):
            ...
            return False, (f"All rows must be lists. Rows {non_list_rows} are not lists. "
                           "Required format: [['col1', 'col2'], ['row1col1', 'row1col2']]")
        ...
        if len(set(col_counts)) > 1:
            return False, (f"All rows must have the same number of columns. "
                           f"Found column counts: {col_counts}. Fix your data structure.")
        ...
        for row_idx, row in enumerate(table_data):
            for col_idx, cell in enumerate(row):
                if cell is None:
                    return False, (f"Cell ({row_idx},{col_idx}) is None. "
                                   "All cells must be strings, use empty string '' for empty cells.")

                if not isinstance(cell, str):
                    return False, (f"Cell ({row_idx},{col_idx}) is {type(cell).__name__}, not string. "
                                   f"All cells must be strings. Value: {repr(cell)}")
    ...
    def validate_text_formatting_params(
        ...
        if all(param is None for param in formatting_params):
            return False, ("At least one formatting parameter must be provided "
                           "(bold, italic, underline, font_size, or font_family)")
    ...
    def validate_index(self, index: int, context: str = "Index") -> Tuple[bool, str]:
        ...
        if index < 0:
            return False, (f"{context} {index} is negative. "
                           "You MUST call inspect_doc_structure first to get the proper insertion index.")
