...
async def export_doc_to_pdf(
    ...
    if mime_type != "application/vnd.google-apps.document":
        return (f"Error: File '{original_name}' is not a Google Doc (MIME type: {mime_type}). "
                f"Only native Google Docs can be exported to PDF.")
    ...
    try:
        ...
        return (f"Successfully exported '{original_name}' to PDF and saved to Drive as '{pdf_filename}' "
                f"(ID: {pdf_file_id}, {pdf_size:,} bytes){folder_info}. "
                f"PDF: {pdf_web_link} | Original: {web_view_link}")

    except Exception as e:
        return (f"Error: Failed to upload PDF to Drive: {str(e)}. "
